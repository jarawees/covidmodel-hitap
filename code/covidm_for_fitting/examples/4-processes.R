...
#' CHANGE THIS to reflect the path to covidm.
.args <- if (interactive()) c(".") else commandArgs(trailingOnly = TRUE)

# covidm options
cm_path = ifelse(length(.args), .args[1], ".");
cm_force_rebuild = F;
cm_build_verbose = T;
cm_version = 2;
...
params = cm_parameters_SEI3R(
    ...
    processes = list(
        list(source = "Ia", type = "multinomial", names = c("test1", "test2"), report = c("ipo", "ipo"),
        ...
        list(source = "Ip", type = "multinomial", names = c("test1", "test2"), report = c("ipo", "ipo"),
             prob = matrix(c(rep(0.1, 16), rep(0.9, 16)), nrow = 2, ncol = 16, byrow = T),
             delays = matrix(c(cm_delay_gamma(7, 7, 60, 0.25)$p, cm_delay_gamma(14, 14, 60, 0.25)$p), nrow = 2, byrow = T)
        ),
        list(source = "Ip", type = "multinomial", names = c("test3", "null"), report = c("o", ""),
             prob = matrix(c(rep(0.1, 16), rep(0.9, 16)), nrow = 2, ncol = 16, byrow = T),
             delays = matrix(c(cm_delay_gamma(7, 7, 60, 0.25)$p, cm_delay_gamma(14, 14, 60, 0.25)$p), nrow = 2, byrow = T)
        )
    )

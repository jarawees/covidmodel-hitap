...
cm_mcmc_likelihood_ = function(x, details, i)
{
    ...
    if (details$fit_sampling == 1) {
        ll = details$likelihood_func(parameters, dynamics, details$data, x)
    } else {
        dynamics = split(dynamics, by = "run");
        ll = sum(sapply(dynamics, function(d) details$likelihood_func(parameters, d, details$data, x), simplify = T)) / details$fit_sampling;
    }
...
cm_fit = function(base_parameters, priors, parameters_func, likelihood_func, data,
                  ...
                  opt_maxeval = 1000, opt_ftol_abs = 1e-6,
                  verbose = T, options = list(
                      ...
                      opt_maxeval = opt_maxeval, opt_ftol_abs = opt_ftol_abs, 
                      verbose = verbose),
                  debug = F)
{
    # Check parameters
    ...
    if (fit_method == "mcmc") {
        ...
        if (options$mcmc_init_opt) {
            ...
            initial = cm_backend_optimize(cm_mcmc_likelihood_, details, priors,
                options$fit_seed, options$opt_maxeval, options$opt_ftol_abs, options$verbose);
            setDT(initial);

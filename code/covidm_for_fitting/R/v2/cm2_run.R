...
# TODO -- in progress... interface likely to change.
cm_sample_fit = function(fit, n, time1 = NULL)
{
    dynamics = NULL;
    rows = sample.int(nrow(fit$posterior), size = n, replace = T);

    # Adjust stopping time
    base_parameters = rlang::duplicate(fit$base_parameters);
    if (!is.null(time1)) {
        base_parameters$time1 = time1;
    }
    ...
    for (i in 1:n)
    {
        ...
        parameters = cm_translate_parameters(fit$parameters_func(base_parameters, x));
...
# TODO -- in progress... interface likely to change.
cm_simulate = function(parameters, n = 1, model_seed = 0, n_threads = 1, file_out = "")
{
    params = cm_translate_parameters(parameters);
    cm_check_parameters(params);
    results = cm_backend_simulate_v2(params, n, model_seed, n_threads, file_out);

    if (!is.null(results$file_out)) {
        return (results);
    }

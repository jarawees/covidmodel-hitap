...
cm_iv_set = function(iv, ymd_iv_first_day, ymd_iv_last_day, ..., fun = "multiply")
{
    ...
    builtin_fun = list(
        ...
        lowerto  = function(x, y) pmin(x, y),
        reduce   = function(x, y) pmin(pmax(0, x - (1 - y)), 1)
    );
    if (!is.function(fun)) {
        if (!fun %in% names(builtin_fun)) {
            stop(paste0("cm_iv_set: fun must be a function(x, y) computing change y's impact on parameter x, or one of ", paste0(names(builtin_fun), collapse = ", "), "."));
        }
        fun = builtin_fun[[fun]];
    }

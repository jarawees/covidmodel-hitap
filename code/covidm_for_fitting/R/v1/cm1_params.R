...
cm_check_parameters = function(parameters)
{
    ...
    for (i in 1:length(parameters$pop))
    {
        ...
        if (!is.null(parameters$pop[[i]]$observer)) {
            if (!(is.function(parameters$pop[[i]]$observer) & length(formals(parameters$pop[[i]]$observer)) == 2)) {
...
#  automatically accounts for clustering of multiple morbidities
cm_high_risk_prevalence <- function(country, aggregate = F){
    cnt <- country
    country_data <- cm_highrisk[country == cnt]
    if(aggregate){
        multimor_corr <- unlist(dcast(country_data, morbidity~age_from, value.var="multimorbid_corr")[1,-"morbidity"])
        morbidities <- dcast(country_data, morbidity~age_from, value.var="prevalence")
        prevmax <- unlist(lapply(morbidities[, -"morbidity"], mean, na.rm=T))
        
        country_data <- country_data[, .(
            lowlim = max(prevalence, na.rm = T),
            uplim = 1-prod(1-prevalence, na.rm = T)
        ), by=c("country", "age_from", "age_to")]
        country_data[, "multimor_corr"] <- multimor_corr
        
        country_data <- country_data[, .(
            highrisk = lowlim + (uplim-lowlim) * (1 - multimor_corr)
        ), by=c("country", "age_from", "age_to")]
    }
    return(country_data)
}

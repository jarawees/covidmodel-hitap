...
# Source: world urbanisation prospects (UN), includes major urban areas in surrounding; Joburg includes Ekurhuleni
popLMIC = rbind(
...
# From worldpop data
wpd = readRDS("~/Dropbox/nCov-2019/data_sources/demographic_data/worldpop_dist.rds");

get_city = function(country, admin_like, cityname)
{
    cit = rlang::duplicate(wpd[country_name == country & adm_name %like% admin_like]);
    cit_total_pop = cit[, sum(total_pop)];
    cit[, M_4_2020 := M_1_2020 + M_4_2020];
    cit[, M_1_2020 := NULL];
    cit[, F_4_2020 := F_1_2020 + F_4_2020];
    cit[, F_1_2020 := NULL];
    names(cit)[8] = "M_0_2020";
    names(cit)[25] = "F_0_2020";
    cit = melt(cbind(adm_name = cit[, adm_name], cit[, M_0_2020:F_80_2020], total_pop = cit[, total_pop]), id.vars = c("adm_name", "total_pop"));
    cit[, pop := total_pop * value / 100];
    cit = cit[, .(pop = sum(pop)), by = variable];
    ages = c("0-4", "5-9", "10-14", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80+");
    data.table(country_code = 7777, name = cityname, age = ages, 
        f = cit[variable %like% "^F", pop / 1000], m = cit[variable %like% "^M", pop / 1000], location_type = 4);
}

popLMIC2 = rbind(
    get_city("India", "DELHI", "India | Delhi (WorldPop)"),
    get_city("Ethiopia", "ADIS ABEBA", "Ethiopia | Addis Ababa (WorldPop)"),
    get_city("Kenya", "NAIROBI", "Kenya | Nairobi (WorldPop)"),
    get_city("South Africa", "JOHANNESBURG|EKURHULENI", "South Africa | Johannesburg (WorldPop)"),
    get_city("Pakistan", "karachi", "Pakistan | Karachi (WorldPop)")
);

populations = rbind(populations, popLMIC2);
...
z_home = contact_matrix(z_survey, age.limits = seq(0, 75, by = 5), survey.pop = z_survey_pop, filter = list(cnt_home = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
z_work = contact_matrix(z_survey, age.limits = seq(0, 75, by = 5), survey.pop = z_survey_pop, filter = list(cnt_work = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
z_scho = contact_matrix(z_survey, age.limits = seq(0, 75, by = 5), survey.pop = z_survey_pop, filter = list(cnt_school = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
z_othe = contact_matrix(z_survey, age.limits = seq(0, 75, by = 5), survey.pop = z_survey_pop, filter = list(cnt_home = 0, cnt_work = 0, cnt_school = 0), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
...
p_pop = populations[name %like% "Peru", f + m, by = age]$V1 * 1000
p_survey_pop = data.frame(lower.age.limit = seq(0, 75, by = 5),
    population = c(p_pop[1:15], sum(p_pop[16:21])))

p_home = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = p_survey_pop, filter = list(cnt_home = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
p_work = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = p_survey_pop, filter = list(cnt_work = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
p_scho = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = p_survey_pop, filter = list(cnt_school = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
p_othe = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = p_survey_pop, filter = list(cnt_home = 0, cnt_work = 0, cnt_school = 0), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
...
v_pop = populations[name %like% "Viet Nam", f + m, by = age]$V1 * 1000
v_survey_pop = data.frame(lower.age.limit = seq(0, 75, by = 5),
    population = c(v_pop[1:15], sum(v_pop[16:21])))

v_home = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = v_survey_pop, filter = list(cnt_home = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
v_work = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = v_survey_pop, filter = list(cnt_work = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
v_scho = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = v_survey_pop, filter = list(cnt_school = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
v_othe = contact_matrix(p_survey, age.limits = seq(0, 75, by = 5), survey.pop = v_survey_pop, filter = list(cnt_home = 0, cnt_work = 0, cnt_school = 0), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
...
# Dodd et al (Zambia, South Africa)
d_survey = get_survey("https://doi.org/10.5281/zenodo.2548692")
d_survey$contacts[, cnt_age_exact := as.character(cnt_age_exact)]
d_survey$contacts[cnt_age_exact %like% "[a-z]", cnt_age_exact := 0.5] # convert ages given in days, weeks, months to 0.5
d_survey$contacts[, cnt_age_exact := as.numeric(cnt_age_exact)]

dz_pop = populations[name %like% "Zambia", f + m, by = age]$V1 * 1000
dz_survey_pop = data.frame(lower.age.limit = seq(0, 75, by = 5),
    population = c(dz_pop[1:15], sum(dz_pop[16:21])))

dz_home = contact_matrix(d_survey, countries = "Zambia", age.limits = seq(0, 75, by = 5), survey.pop = dz_survey_pop, filter = list(cnt_home = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
dz_work = contact_matrix(d_survey, countries = "Zambia", age.limits = seq(0, 75, by = 5), survey.pop = dz_survey_pop, filter = list(cnt_work = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
dz_scho = contact_matrix(d_survey, countries = "Zambia", age.limits = seq(0, 75, by = 5), survey.pop = dz_survey_pop, filter = list(cnt_school = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
dz_othe = contact_matrix(d_survey, countries = "Zambia", age.limits = seq(0, 75, by = 5), survey.pop = dz_survey_pop, filter = list(cnt_home = 0, cnt_work = 0, cnt_school = 0), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix

rownames(dz_home) = colnames(dz_home) = limits_to_agegroups(seq(0, 75, by = 5))
rownames(dz_work) = colnames(dz_work) = limits_to_agegroups(seq(0, 75, by = 5))
rownames(dz_scho) = colnames(dz_scho) = limits_to_agegroups(seq(0, 75, by = 5))
rownames(dz_othe) = colnames(dz_othe) = limits_to_agegroups(seq(0, 75, by = 5))


ds_pop = populations[name %like% "South Africa", f + m, by = age]$V1 * 1000
ds_survey_pop = data.frame(lower.age.limit = seq(0, 75, by = 5),
    population = c(ds_pop[1:15], sum(ds_pop[16:21])))

ds_home = contact_matrix(d_survey, countries = "South Africa", age.limits = seq(0, 75, by = 5), survey.pop = ds_survey_pop, filter = list(cnt_home = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
ds_work = contact_matrix(d_survey, countries = "South Africa", age.limits = seq(0, 75, by = 5), survey.pop = ds_survey_pop, filter = list(cnt_work = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
ds_scho = contact_matrix(d_survey, countries = "South Africa", age.limits = seq(0, 75, by = 5), survey.pop = ds_survey_pop, filter = list(cnt_school = 1), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix
ds_othe = contact_matrix(d_survey, countries = "South Africa", age.limits = seq(0, 75, by = 5), survey.pop = ds_survey_pop, filter = list(cnt_home = 0, cnt_work = 0, cnt_school = 0), symmetric = F, estimated.contact.age = "sample", missing.contact.age = "sample")$matrix

rownames(ds_home) = colnames(ds_home) = limits_to_agegroups(seq(0, 75, by = 5))
rownames(ds_work) = colnames(ds_work) = limits_to_agegroups(seq(0, 75, by = 5))
rownames(ds_scho) = colnames(ds_scho) = limits_to_agegroups(seq(0, 75, by = 5))
rownames(ds_othe) = colnames(ds_othe) = limits_to_agegroups(seq(0, 75, by = 5))
